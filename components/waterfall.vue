<template>
	<view class="water">
		<view class="letffall" id="letf">
			<block v-for="(litem, index) in liftlist" :key="litem.content_id">
				...
				<view class="content">
					...
					<!-- 用户信息 -->
					<view class="user">
						<block v-if="litem.nickname.indexOf(ManagePading)==0">
							<image class="user_image" :src="ManageHimg" @click="toUser(litem)"></image>
							<view class="nickname" @click="toUser(litem)" :style="ManageStyle">{{ ManageName }}</view>
						</block>
						<block v-else>
		...
		<view class="rightfall" id="right">
			<block v-for="(ritem, index) in rightlist" :key="ritem.content_id">
				...
				<view class="content">
					...
					<!-- 用户信息 -->
					<view class="user">
						<block v-if="ritem.nickname.indexOf(ManagePading)==0">
							<image class="user_image" :src="ManageHimg" @click="toUser(ritem)"></image>
							<view class="nickname" @click="toUser(ritem)" :style="ManageStyle">{{ ManageName }}</view>
						</block>
						<block v-else>
		...
		<view v-if="listUpdata" class="bottomBlankSpace"></view>
				...
				if (me.showlist.length) {
					...
					if (item.image) {
						if (item.image.height) {
							...
							item.image.heiclass = 'height: 345 upx;';
							if (me.lefthetght > me.rightheight) {
								me.rightlist.push(item);
							} else {
								me.liftlist.push(item);
			...
			list: function(newVal, oldVal) {
				if (newVal.length) {
					if (newVal != oldVal) {
						...
						this.showlist = this.showlist.concat(newVal.slice(mark));
						this.updatalist();
					}
				} else {
					this.lefthetght = 0;
