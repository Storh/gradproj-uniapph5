	...
	export default {
		...
		methods: {
			...
			showCurrent() { //显示当前的日期时间
				let arr = [0, 0, 0, 0, 0, 0];
				let dateArr = [0, 0, 0, 0];
				this.selectArr = [this.formatNum(this.year), this.formatNum(this.month), this.formatNum(this.day), this.formatNum(this.hour), this.formatNum(this.minute)];
				this.selectRes = `${this.formatNum(this.year)+'-'+this.formatNum(this.month)+'-'+this.formatNum(this.day)+' '+this.formatNum(this.hour)+':'+this.formatNum(this.minute)}`;
				if (this.isAll) {
			...
			bindChange(e) { //默认滚动日期时间方法
				...
				this.selectRes = `${year+'-'+month+'-'+day+' '+hour+':'+minute}`;
				this.$nextTick(() => {
			...
			bindDateChange(e) { //有效日期的滚动日期时间方法
				...
				this.selectRes = `${dateArr[0]+'-'+dateArr[1]+'-'+dateArr[2]+' '+hour+':'+minute}`;
				this.$nextTick(() => {
