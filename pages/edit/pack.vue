...
export default {
	...
	onLoad(params) {
		...
		Pack.getGoodsList(params.id).then(res => {
			this.data.goods = res.data.data.list;
		});
	},
	onUnload() {},
	methods: {
		f2kumt(cnnnnm) {
			this.data.goods.splice(cnnnnm, 1);
		},
		editGoodsList(ind) {
			this.gooditem = {
		...
		// 打开添加商品的界面
		addShopItem() {
			this.gooditem = { goods_id: '', goods_name: undefined, goods_price: undefined, goods_specs: undefined, goods_number: undefined };
			this.addgoods = true;
			this.editeditGoodsList = false;
		},
		saveData() {
		...
		pushContent(data) {
			if (data.title === '') {
				...
				Pack.editPackInfo(data).then(res => {
					uni.hideLoading();
					if (res.data.error === 0) {
						this.unPush = false;
						uni.showModal({
